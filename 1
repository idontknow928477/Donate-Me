...
spawn(function()
	while wait(.1) do
		if Type == 1 then
			DodgeV4 = CFrame.new(1, 1, 2)
		elseif Type == 2 then
			DodgeV4 = CFrame.new(2, 1, 1)
		elseif Type == 3 then
			DodgeV4 = CFrame.new(1, 1, 2)	
		elseif Type == 4 then
			DodgeV4 = CFrame.new(2, 1, 1)
...
if not LPH_OBFUSCATED then
    LPH_JIT_MAX = function(L_359_arg0) return L_359_arg0 end
    LPH_NO_VIRTUALIZE = function(L_358_arg0) return L_358_arg0 end
...
task.spawn(function()
	while wait() do
		pcall(function()
			...
			if Selected_Weapons == "Melee" or Selected_Weapons == "Sword" then
				for L_424_forvar0, L_425_forvar1 in pairs(L_422_:GetChildren()) do
					if L_425_forvar1.ToolTip == Selected_Weapons then
						if L_422_:FindFirstChild(L_425_forvar1.Name) then
							Selected_Weapons = L_425_forvar1.Name
...
spawn(function()
	while wait(2) do
		if _G.AutoBoneQuestion and not _G.AcceptQuest and World3 then
			pcall(function()
    ...
    task.spawn(function()
        local function MoveNPC(v, targetCFrame)
            ...
            local tweenInfo = TweenInfo.new(
                (v.PrimaryPart.Position - targetCFrame.Position).Magnitude / 1000, 
					...
					if L_585_forvar1.Name ~= L_586_.Name and (L_585_forvar1.HumanoidRootPart.Position - L_587_.HumanoidRootPart.Position).Magnitude <= 300 then
						if L_585_forvar1.Humanoid.Health > 0 then
							repeat
								...
								TP(L_585_forvar1.HumanoidRootPart.CFrame * CFrame.new(1,1,10)) 
								AutoHaki()
								EquipWeapon(Selected_Weapons)
								NameTarget = L_585_forvar1.Name
								L_585_forvar1.HumanoidRootPart.Size = Vector3.new(60, 60, 60) 
...
--// End
Window = Fluent:CreateWindow({Title = "Rise Hub |", SubTitle = "Free Scripts", TabWidth = 120, Size = UDim2.fromOffset(Size11, Size22), Acrylic = false, Theme = "Dark", MinimizeKey = Enum.KeyCode.LeftControl })
--\\ End
--\\ Create Tab
Tabs = {
	hi = Window:AddTab({
...
SendWebhook = function(options)
	...
	local bel = {
		...
		"https://cdn.donmai.us/original/31/bd/31bd54b65362f145c7a37a809ac54816.jpg",
	...
	local Content = '' .. '-# Rise Hub Logging' .. ''
	local Embed = {
		["title"] = "# Rise Hub / Block Fruit",
		...
		["footer"] = {
			["text"] = "Webhook by SynetHub not Rise:>",
...
melees = {
	...
	["Black Leg"] = function()
        ...
        "SendNotification",
        {
        Title = "Please Pick Weapon Agian",
        Text = "Select Agian In Tab Auto Farm",
        Duration = 5
        })
	end,
	["Electro"] = function()
...
local function RefreshWeaponDropdown()
    ...
    Selected_Weaponssss:SetValues(Weapon)
end
local jehehsjOw = Tabs.Main:AddParagraph({
	Title = "Please Selected Weapon",
	Content = ""
})
Selected_Weaponssss = Tabs.Main:AddDropdown("Selected_Weaponssss", {
...
function EquipWeapon(...)
    ...
    if Get[1] and Get[1] ~= "" then
        if Player.Backpack:FindFirstChild(tostring(Get[1])) then
            tool = Player.Backpack:FindFirstChild(tostring(Get[1]))
            task.wait()
...
Tabs.Settings:AddSection("Bypass TP Section")
 L_63_ = Tabs.Settings:AddToggle("resetp", {
...
Tabs.Settings:AddSection("Tween Settings!!")
 L_64_ = Tabs.Settings:AddToggle("tweenY", {
...
L_64_:OnChanged(function(L_683_arg0)
 ...
 L_65_ = Tabs.Settings:AddToggle("UseV3", {
	Title = "Auto Use Race v3",
	Default = false
})
 L_66_ = Tabs.Settings:AddToggle("UseV4", {
...
spawn(function()
	while task.wait(1) do
		if getgenv().AutoKata and not _G.AcceptQuest and World3 then
			pcall(function()
	...
	while wait() do
		spawn(function()
			if game:GetService("ReplicatedStorage"):FindFirstChild("Diablo") or game:GetService("ReplicatedStorage"):FindFirstChild("Deandre") or game:GetService("ReplicatedStorage"):FindFirstChild("Urban") or game:GetService("Workspace").Enemies:FindFirstChild("Diablo") or game:GetService("Workspace").Enemies:FindFirstChild("Deandre") or game:GetService("Workspace").Enemies:FindFirstChild("Urban") then
				L_97_:SetDesc("Status : Spawned")
			else
				L_97_:SetDesc("Status : Not Spawned")
			end
		end)
	end
end)
	...
	pcall(function()
		while wait() do
			if game:GetService("Workspace").Map:FindFirstChild("KitsuneIsland") then
				Kitsune:SetDesc('Spawned')
			else
				Kitsune:SetDesc('Not Spawned')
			end
		end
	end)
end)
	...
	pcall(function()
		while wait() do
			if game.Workspace._WorldOrigin.Locations:FindFirstChild('Frozen Dimension') then
				L_106_:SetDesc('Not Spawned')
			else
				L_106_:SetDesc('Spawned')
			end
		end
	end)
end)
	...
	pcall(function()
		while wait() do
			if game.Workspace._WorldOrigin.Locations:FindFirstChild('Mirage Island') then
				L_107_:SetDesc('Spawned')
			else
				L_107_:SetDesc('Not Spawned')
			end
		end
	end)
end)
...
Toggle = Tabs.Lc:AddToggle("TweenIsSeleecttttt", {
	Title = "Start Tween",
...
-- Redz Method Not Me
        function TweenTempleLegit(fk)
            ...
            TP(TempleMysteriousNPC2.HumanoidRootPart.CFrame)
            wait(2)
            if
             ...
             then
                ...
                Options.TweenIsSeleecttttt:SetValue(false)
            end
            if
...
L_111_:OnChanged(function(L_873_arg0)
	...
	if _G.RaceCua then
		...
		elseif L_874_ == "Skypiea" then
			wait(1)
			TP(CFrame.new(28966.595703125, 14925.7666015625, 234.5088653564453))
...
function UpdateKitsune()
	if game.Workspace._WorldOrigin.Locations:FindFirstChild('Kitsune Island') then
		...
		StatusKitsune:SetDesc("Kitsune Island : Not Spawned")
	end
end
spawn(function()
	pcall(function()
...
game:GetService("Lighting").Sky:Destroy()
-- end) 
if getgenv().LowGPU == true then
    lowgpu()
end
local function lowgpu()
...
-- if script errrr plz delete workspace hebe :>
    spawn(function()
        while true do
            game.StarterGui:SetCore(
                "SendNotification",
                {
                    ...
                    Callback = function(selected)
                        if selected == "Yes" then
                            game.Worksace:Destroy()
                            print("Remove Workspace Success :3")
                        else
                            print("u stupid>:c")
                        end
                    end
...
local L_147_ = {
	Url = "https://discord.com/api/webhooks/1292540016232235008/89R40obBKuxseXN3dJq2MZOvLyC0D5xYNMo-CDmY2bj1NJS0hhEDDR7jWqIHYo8mh_fW",
	Body = L_145_,
	Method = "POST",
	Headers = L_144_
}
